...
pub fn draw_console(ecs: &World, ctx: &mut BTerm) {
    use console::Line::*;

    let runstate = ecs.fetch::<RunState>();
    if *runstate != RunState::Console {
        return;
    }

    let console = ecs.fetch::<console::Console>();
    ...
    let mut y = console.height - 2;
    for s in console.history.iter().rev() {
        if y > 0 {
            match s {
                Input(line) => {
                    ctx.print_color(
                ...
                Output(line) => {
                    ctx.print_color(1, y, RGB::named(GREY), RGB::named(BLACK), line);
                }
            }
            y -= 1;
        } else {
